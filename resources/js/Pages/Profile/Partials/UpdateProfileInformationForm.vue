		...
		<form @submit.prevent="form.patch(route('profile.update'))" class="mt-6 space-y-6">
			...
			<div v-if="props.mustVerifyEmail && user.email_verified_at === null">
				...
				<div v-show="props.status === 'verification-link-sent'" class="mt-2 font-medium text-sm text-green-600 dark:text-green-400">
					Un nuevo correo de verificación ha sido enviado a tu dirección de correo.
				</div>
			</div>
			...
			<div class="flex items-center gap-4">
				...
				<Transition enter-from-class="opacity-0" leave-to-class="opacity-0" class="transition ease-in-out">
					<p v-if="form.recentlySuccessful" class="text-sm text-gray-600 dark:text-gray-400">Información guardada</p>
				</Transition>
			</div>
		</form>
	</section>
